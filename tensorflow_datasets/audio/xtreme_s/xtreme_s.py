...
_DESCRIPTIONS = {
    "fleurs": (
        "FLEURS is the speech version of the FLORES machine translation"
...
class XtremeS(tfds.core.GeneratorBasedBuilder):
    ...
    if self.builder_config.dataset_name == "fleurs":
      features = tfds.features.FeaturesDict({
          "id": tfds.features.Scalar(
              tf.dtypes.int32,
              doc=(
                  "Source text identifier, consistent across all languages"
          ...
          "num_samples": tfds.features.Scalar(
              tf.dtypes.int32, doc="Total number of frames in the audio"
          ),
          "path": tf.dtypes.string,
          "audio": tfds.features.Audio(sample_rate=16_000),
          "transcription": tfds.features.Text(doc="Normalized transcription."),
          "raw_transcription": tfds.features.Text(
              doc="Raw Transcription from FLoRes."
          ),
          "gender": tfds.features.ClassLabel(names=["male", "female", "other"]),
          "lang_id": tfds.features.ClassLabel(names=languages),
          "language": tfds.features.Text(
              doc=(
                  "Language encoded as lowercase, underscore-separated"
                  "version of a BCP-47 tag."
              )
          ),
          "lang_group_id": tfds.features.ClassLabel(
  ...
  def _fleurs_split_generators(self, dl_manager):
    ...
    if self.builder_config.lang_name == "all":
      ...
      data_urls = {
          self.builder_config.lang_name: data_url_format.format(
    ...
    def create_split_generator(
        split: str, file_prefix: str
