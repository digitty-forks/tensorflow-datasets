...
def test_collection_citation_section(
  ...
  expected_citation = textwrap.dedent(
      f"""
            ```
...
def test_collection_versions_section(
  ...
  assert versions_section.content(loader=dummy_dc_loader) == '\n'.join(
      expected_lines
  )
...
def test_collection_datasets_section(
  ...
  expected_lines = [
      (
          '    * `a`:'
          ' [`a/c:1.3.5`](https://www.tensorflow.org/datasets/catalog/a#c)'
      ),
      (
          '    * `b`:'
          ' [`b/d:2.4.8`](https://www.tensorflow.org/datasets/catalog/b#d)'
      ),
      (
          '    * `c`:'
          ' [`c/e:3.5.7`](https://www.tensorflow.org/datasets/catalog/c#e)'
      ),
  ]
  assert datasets_section.content(loader=dummy_dc_loader) == '\n'.join(
      expected_lines
  )
...
def test_get_collection_markdown_string(
  ...
  assert (
      '[`c/e:3.5.7`](https://www.tensorflow.org/datasets/catalog/c#e)' in doc
  )  # Collection dataset.
