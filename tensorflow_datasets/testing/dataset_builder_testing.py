...
class DatasetBuilderTestCase(
  ...
  def tearDownClass(cls):
    super().tearDownClass()
    cls._available_cm.__exit__(None, None, None)  # pylint: disable=protected-access
  ...
  def _prepare_download_results(
      self,
      downloaded_files: Union[Mapping[str, str], str],
  ...
  def dataset_class(self):
    if self.DATASET_CLASS is None or not issubclass(
        self.DATASET_CLASS, dataset_builder.DatasetBuilder
    ):
      raise ValueError(
          "DATASET_CLASS is not a subclass of DatasetBuilder. Call setUpClass()"
          " properly."
      )
